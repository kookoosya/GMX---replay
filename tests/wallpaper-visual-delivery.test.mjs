/**
 * Wallpaper visual delivery — Themes V5 sitev5 / extskin_v5 paths.
 */
...
test("active catalog URLs use sitev5 and extskin_v5 not legacy packs", () => {
  ...
  for (const entry of extCatalog.slice(0, 10)) {
    ...
    assert.match(extThumb, /\/assets\/extskins\/thumbs\/extskin_v5_\d{3}\.webp\?v=/);
    assert.match(extFull, /\/assets\/extskins\/extskin_v5_\d{3}\.webp\?v=/);
...
test("site manifest paths match on-disk sitev5 assets", () => {
...
test("service worker cache version bumped for Themes V5", () => {
...
test("production thumbnail bytes match git when served (no-store)", { skip: process.env.GMX_PROD_WALLPAPER_VERIFY !== "1" }, async () => {
  ...
  if (mismatches > 0) {
    assert.fail(`production not yet on sitev5 pack (${mismatches}/${SAMPLE} thumb hashes differ; deploy pending)`);
  }
});
