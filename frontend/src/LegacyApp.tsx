...
const LEGACY_TABS = new Set(["home", "gm", "gn", "referrals", "leaderboard", "themes", "extthemes", "wallet", "admin"]);

function getSavedLegacyTab() {
  try {
    const raw = String(localStorage.getItem("gmx_last_tab") || "").trim();
    return LEGACY_TABS.has(raw) ? raw : "home";
  } catch {
    return "home";
  }
}
...
function tabFromPath(pathname: string) {
  ...
  if (raw === "/" || raw === "/app" || raw.startsWith("/app/")) return getSavedLegacyTab();
  return "home";
}
...
export default function LegacyApp() {
  ...
  useEffect(() => {
    ...
    // Make the full site i18n catalog available to the legacy runtime inside Vite.
    try {
