/** Admin session helpers (SQLite-backed). */

export function createAdminSessionHelpers({ db, crypto, adminSessionHours }) {
  ...
  function adminSessionCreate(handle) {
    ...
    const expires_at = new Date(Date.now() + adminSessionHours * 60 * 60 * 1000).toISOString();
    db.prepare("INSERT INTO admin_sessions(token, handle, created_at, expires_at) VALUES(?,?,?,?)").run(
      token,
      handle,
      created_at,
      expires_at
