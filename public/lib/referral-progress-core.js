(function (global) {
  if (global.GMXReferralProgressCore) return;

  const REF_UNLOCK_LADDER = Object.freeze([1, 3, 7, 15, 30, 50, 100]);
  ...
  function previousReferralUnlockAt(nextStep) {
  ...
  function neededForNextUnlock(eligible, nextStep) {
    const e = Math.max(0, Number(eligible || 0) || 0);
  ...
  function referralProgressPct(eligible, nextStep, prevStep) {
    const next = Number(nextStep || 0) || 0;
  ...
  function referralProgressState(eligible) {
    const e = Math.max(0, Number(eligible || 0) || 0);
  ...
  global.GMXReferralProgressCore = {
    REF_UNLOCK_LADDER,
