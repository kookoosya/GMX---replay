(function (window) {
  if (window.__GMXWalletPayFactory) return;

  window.__GMXWalletPayFactory = function createGMXWalletPay(ctx) {
    ctx = ctx || {};
    const api = typeof ctx.api === "function" ? ctx.api : async () => ({});
    const getBilling = typeof ctx.getBilling === "function" ? ctx.getBilling : () => ({ receiver: "" });
    const getSelectedCurrency =
      typeof ctx.getSelectedCurrency === "function" ? ctx.getSelectedCurrency : () => "SOL";
    const getWallet =
      typeof ctx.getWallet === "function" ? ctx.getWallet : () => ({ connected: false, publicKey: null });
    const wsChain = ctx.wsChain || "solana:mainnet";
    const b58encode = typeof ctx.b58encode === "function" ? ctx.b58encode : () => "";
    const addIntentMemoInstruction =
      typeof ctx.addIntentMemoInstruction === "function" ? ctx.addIntentMemoInstruction : () => {};
    const getRpcUrl = typeof ctx.getRpcUrl === "function" ? ctx.getRpcUrl : () => "";
    const rpcCandidates =
      typeof ctx.rpcCandidates === "function" ? ctx.rpcCandidates : () => [];
    const shouldRetryRpc =
      typeof ctx.shouldRetryRpc === "function" ? ctx.shouldRetryRpc : () => false;
    ...
    async function ensureBrowserBuffer() {
      const existing =
        typeof globalThis !== "undefined" && globalThis.Buffer ? globalThis.Buffer : null;
      if (existing && typeof existing.from === "function" && typeof existing.alloc === "function")
        return existing;
      ...
      if (web3Buffer && typeof web3Buffer.from === "function") {
        try {
          window.Buffer = web3Buffer;
        } catch (_e) {}
        try {
          globalThis.Buffer = web3Buffer;
        } catch (_e) {}
        return web3Buffer;
      }
      if (!bufferReady) {
        class MiniBuffer extends Uint8Array {
          ...
          static alloc(size) {
            return new MiniBuffer(Number(size) || 0);
          }
          ...
          static concat(list) {
            ...
            for (const item of arr) {
              const chunk = MiniBuffer.from(item);
              out.set(chunk, off);
              off += chunk.length;
            }
            return out;
        ...
        bufferReady = Promise.resolve(MiniBuffer).then((B) => {
          try {
            window.Buffer = B;
          } catch (_e) {}
          try {
    ...
    async function ensureSplToken() {
      ...
      const getAssociatedTokenAddress = async (
        ...
        const out = web3.PublicKey.findProgramAddressSync(
          [toPkBytes(owner), toPkBytes(tokenProgramId), toPkBytes(mint)],
    ...
    async function buildPaymentTx(intent) {
      ...
      if (String(intent.currency || getSelectedCurrency()) === "SOL") {
      ...
      const payerAta = await spl.getAssociatedTokenAddress(
        mint,
        payer,
        false,
      ...
      const receiverAta = await spl.getAssociatedTokenAddress(
        mint,
        receiver,
        false,
        spl.TOKEN_PROGRAM_ID,
      ...
      if (!recvInfo && typeof connection.getMinimumBalanceForRentExemption === "function") {
        try {
          neededLamports += BigInt(String(await connection.getMinimumBalanceForRentExemption(165)));
        } catch (_e) {}
      }
      const payerLamports = BigInt(String(await connection.getBalance(payer).catch(() => 0)));
    ...
    async function walletSendTransaction(tx, connection) {
      ...
      if (WALLET.kind === "standard") {
        ...
        if (typeof featSend === "function") {
          const out = await featSend({ transaction: tx, account: acc, chain: wsChain });
        ...
        if (typeof featSign === "function") {
          const out = await featSign({ transaction: tx, account: acc, chain: wsChain });
