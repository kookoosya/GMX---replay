#!/usr/bin/env node
/**
 * OBSOLETE — archived VPS SSH deploy. Production is Render-only.
 * Run via: npm run deploy:legacy:vps  (or DEPLOY_VPS_ALLOW=1)
 */
if (process.env.DEPLOY_VPS_ALLOW !== "1") {
  console.error("[deploy] VPS deploy is archived. Set DEPLOY_VPS_ALLOW=1 or use npm run deploy:legacy:vps");
  console.error("See tools/legacy/README.md");
  process.exit(1);
}

/**
 * Deploy GMXReply to VPS via SSH (legacy).
 *
 * Usage:
 *   DEPLOY_VPS_ALLOW=1 DEPLOY_SSH_HOST=root@host DEPLOY_SSH_PASSWORD=*** node tools/legacy/deploy-vps.mjs probe
 */
...
async function detectAppDir(conn) {
  ...
  throw new Error(`Could not find app dir — run: npm run deploy:legacy:setup (or set DEPLOY_APP_DIR)`);
}
...
async function probe(conn) {
  ...
  try {
    ...
    console.log(`\n[deploy] App not installed yet. Run: npm run deploy:legacy:setup\n`);
    throw e;
  }
}
