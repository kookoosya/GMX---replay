#!/usr/bin/env node
/** Emit Themes V4 paths, counts, and wallpaper-core.js to public bundles. */
...
function patchAppWallpapers(file) {
  ...
  src = src.replace(/id: `extv3_\$\{String\(i\)\.padStart\(3, "0"\)\}`/g, 'id: `extskin_${String(i).padStart(3, "0")}`');

  src = src.replace(
    /function formatExtPackIdLocal\(n\) \{[\s\S]*?\n    \}/,
  ...
  src = src.replace(/formatExtPackIdLocal/g, "formatExtSkinIdLocal");

  src = src.replace(
    /function normalizeExtWallpaperIdLocal\(id, catalog\) \{[\s\S]*?\n    \}/,
  ...
  src = src.replace(/if \(norm\.startsWith\("extv3_"\)\)/g, 'if (norm.startsWith("extskin_"))');
  src = src.replace(/\/assets\/extbg\//g, "/assets/extskins/");
  src = src.replace(/pexels100_portrait_001/g, "extskin_v4_001");
  src = src.replace(/if \(norm\.startsWith\("extv3_"\)\)/g, 'if (norm.startsWith("extskin_"))');
...
function bumpRevAndSw() {
  for (const rel of ["public/app.js", "frontend/public/app.js", "extension/lib/ext-config.js"]) {
  ...
  for (const rel of ["public/sw.js", "frontend/public/sw.js"]) {
    ...
    src = src.replace(/const CACHE = "gmx-shell-v\d+"/, 'const CACHE = "gmx-shell-v4"');
    if (!src.includes("gmx-shell-v3")) {
      src = src.replace(/const LEGACY_CACHES = \[/, 'const LEGACY_CACHES = ["gmx-shell-v3", ');
    }
    src = src.replace(
      /function isWallpaperFullAsset\(pathname\) \{[\s\S]*?\n\}/,
  ...
  fs.writeFileSync(pwaCore, fs.readFileSync(pwaCore, "utf8").replace(/gmx-shell-v3/g, "gmx-shell-v4"), "utf8");
}
...
bumpRevAndSw();
console.log(`emit-themes-v4 OK (site ${WALLPAPER_PACK_COUNT}, ext skins ${EXT_SKIN_PACK_COUNT}, rev ${ASSET_REV})`);
