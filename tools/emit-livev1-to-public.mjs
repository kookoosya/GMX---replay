#!/usr/bin/env node
/** Emit Live V1 paths, counts, and wallpaper-core.js to public bundles. */
...
function bumpRevAndSw() {
  ...
  let pwa = fs.readFileSync(pwaCore, "utf8");
  if (!pwa.includes('gmx-shell-v5"')) {
    pwa = pwa.replace(/export const PWA_CACHE_NAME = "gmx-shell-v\d+"/, 'export const PWA_CACHE_NAME = "gmx-shell-v5"');
    if (!pwa.includes("gmx-shell-v4")) {
      pwa = pwa.replace(/export const PWA_LEGACY_CACHES = \[/, 'export const PWA_LEGACY_CACHES = ["gmx-shell-v4", ');
    }
  }
  fs.writeFileSync(pwaCore, pwa, "utf8");
}
...
bumpRevAndSw();
console.log(`emit-livev1 OK (site ${WALLPAPER_PACK_COUNT}, ext ${EXT_SKIN_PACK_COUNT}, rev ${ASSET_REV})`);
