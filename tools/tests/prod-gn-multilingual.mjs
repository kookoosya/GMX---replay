#!/usr/bin/env node
/**
 * Production-safe GN multilingual verification (isolated test handle, max 10 credits).
 */
...
function enProbe(t) {
  ...
  if (!s) return false;
  return passesGnSemantics(s, "en");
}

const SCENARIOS = [
  { id: "en_mid_classic", lang: "en", mode: "mid", style: "classic", probe: enProbe },
  { id: "ru_mid_classic", lang: "ru", mode: "mid", style: "classic", probe: (t) => /[\u0400-\u04FF]/.test(t) && passesGnSemantics(t, "ru") },
  { id: "tr_mid_cheer", lang: "tr", mode: "mid", style: "cheer", probe: (t) => /[çğıöşüÇĞİÖŞÜ]|gece|uyku|geceler/i.test(t) },
  { id: "es_mid_noemoji", lang: "es", mode: "mid", style: "noemoji", probe: (t) => /[áéíóúñ]|noche|descans/i.test(t), noEmoji: true },
  ...
  { id: "en_min_classic", lang: "en", mode: "min", style: "classic", probe: enProbe, min: true },
  { id: "ru_min_classic", lang: "ru", mode: "min", style: "classic", probe: (t) => /[\u0400-\u04FF]/.test(t), min: true },
  { id: "en_mid_degen", lang: "en", mode: "mid", style: "degen", probe: enProbe, styleSwitch: true },
  { id: "en_mid_noemoji", lang: "en", mode: "mid", style: "noemoji", probe: enProbe, noEmoji: true, recovery: true },
];
...
async function generate(token, { lang, mode, style, kind = "gn" }) {
...
console.log(`Prod GN multilingual: ${BASE}\n`);

const handle = freshSmokeHandle("gnpm");
...
for (const sc of SCENARIOS) {
  ...
  if (hasMorningSemantics(reply)) fail(`${sc.id}: morning semantics: ${reply}`);
  if (sc.noEmoji && RE_ANY_EMOJI.test(reply)) fail(`${sc.id}: emoji in noemoji`);
  if (sc.min && !passesMinSubstance(reply, "gn", sc.lang, sc.style)) fail(`${sc.id}: thin min: ${reply}`);
  if (!passesGnSemantics(reply, sc.lang)) fail(`${sc.id}: GN semantics: ${reply}`);
