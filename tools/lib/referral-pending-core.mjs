/** Pending referral attribution — shared between site client and tests. */

export const REF_PENDING_STORAGE_KEY = "gmx_ref_pending_v1";
export const REF_PENDING_SCHEMA_VERSION = 1;
/** 7 days — matches server/config REF_PENDING_TTL_MS */
export const REF_PENDING_TTL_MS = 7 * 24 * 60 * 60 * 1000;
export const REF_CODE_MIN_LENGTH = 6;
export const REF_CODE_MAX_LENGTH = 32;
...
export function createPendingRecord(code, opts = {}) {
...
export function isPendingValid(record, now = Date.now()) {
...
export function parsePendingRecord(raw, now = Date.now()) {
  if (!raw) return null;
  let parsed;
  try {
...
export function serializePending(record) {
  return JSON.stringify(record);
}

/**
 * First-touch capture policy before server confirmation.
 * Reload with same code does not extend TTL or re-fire click analytics.
 */
export function resolvePendingCapture(existing, queryCode, now = Date.now(), ttlMs = REF_PENDING_TTL_MS) {
  ...
  const created = createPendingRecord(next, { now: nowMs, ttlMs, source: "ref_query" });
  if (!created) {
...
/** URL ref wins at connect time; otherwise use valid pending record. */
export function resolveRefForInit(pendingRecord, queryCode, now = Date.now()) {
...
export function pendingRecordHasSecrets(record) {
  if (!record || typeof record !== "object") return false;
  const forbidden = [
    "token",
    "handle",
